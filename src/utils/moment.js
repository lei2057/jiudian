var Moment = function (date) {
  // var date
  if (date) { this.date = new Date(date) } else { this.date = new Date() }
  return this
}
/**
...
Moment.prototype.format = function (format) {
  ...
  if (/(y+|Y+)/.test(format)) { format = format.replace(RegExp.$1, (date.getFullYear() + '').substr(4 - RegExp.$1.length)) }
  if (/(E+)/.test(format)) { format = format.replace(RegExp.$1, ((RegExp.$1.length > 1) ? (RegExp.$1.length > 2 ? '/u661f/u671f' : '/u5468') : '') + week[date.getDay() + '']) }
  for (var k in o) {
